 ...
 * Parameter: <gpio>
 */
bool exec_ds18b20(int gpio, struct ds18b20_t* results, int max_results, int* found_results)
{
	int r, i, n=0;
	uint8_t data[12];
  struct ds18b20_t *device;
  *found_results = 0;
  if (!is_valid_gpio_pin(gpio) || !results || max_results < 1 || max_results > 8) {
    return false;
  }
  ...
  do {
    ...
    dbg( "%02x%02x%02x%02x%02x%02x%02x%02x %c%d.%02d\n", 
      addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], addr[6], addr[7],
      tSign, tVal, tFract);
    device->tSign = tSign;
    device->tVal = tVal;
    device->tFract = tFract;
    device ++;
    n ++;
  } while (n < max_results);
  *found_results = n;
